...
- `frontend_forecast_annual_model_matrix.csv`
- `frontend_long_term_forecast_endpoint.csv`
- `frontend_long_term_forecast_overall.csv`
- `frontend_long_term_forecast_metadata.csv`
...
- `frontend_excess_allcause_daily.csv`: modeled heat-attributable excess for all-cause ED visits (`ed_visits`), EMS calls (`ems_calls`), and deaths (`deaths`) by community-area-day. It includes temperature, tree canopy coverage, population when available, modeled positive excess events, excess rate per 100,000 residents, unit cost, and estimated cost. Modeled fractional excess (`excess_events_heat`) and its derived rate/cost fields are retained; observed small-cell counts remain suppressed.
...
Build long-term annual forecast exports with:

```r
source("code/13_build_long_term_forecast_exports.R")
```
