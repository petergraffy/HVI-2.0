...
The dashboard also includes an annual planning layer for 5-, 10-, and 20-year horizons. This layer aggregates the daily community-area model matrix to annual temperature and endpoint outcomes, estimates endpoint-specific annual count models against annual mean daily maximum temperature, and applies a temperature trend from the longest available Chicago climate panel. Forecast exports are generated by [code/13_build_long_term_forecast_exports.R](code/13_build_long_term_forecast_exports.R).

The resulting frontend exports are intended for long-term planning views: estimated annual endpoint burden, rate changes, overall forecast risk, and dominant endpoint under future annual-temperature scenarios. These forecasts are planning scenarios, not deterministic climate projections or daily alert forecasts.
