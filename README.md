...
6. Provide dashboard-ready scenario outputs so users can explore how temperature, greenness, cooling access, and other modifiable covariates may affect endpoint-specific risk.
...
Other exposure or adaptive-capacity inputs include humidity where available, NDVI, tree canopy coverage, PM2.5, NO2, and air-conditioning probability.
...
- NDVI, tree canopy coverage, and other built or natural environment measures
...
- AC prevalence loss relative to baseline, including low-prevalence outage-like scenarios

Scenario exports are generated by [code/12_build_scenario_exports.R](code/12_build_scenario_exports.R). The script converts Fahrenheit to Celsius for model scoring, holds humidity at baseline by default, applies NDVI changes and AC loss-of-cooling scenarios from each community area's baseline, carries tree canopy baseline values for frontend context, caps heat dose at endpoint-specific observed support, and marks model-boundary rows using `heat_dose_capped` and `prediction_capped_for_display`.

Humidity is retained in the model matrix for future sensitivity analyses, but it is not active in the public scenario slider by default because the current humidity-adjusted grid produced unstable and hard-to-interpret scenario behavior.

For public visualization, the app should use the 0-100 score fields rather than interpreting scenario predicted counts as causal estimates. NDVI and AC slider effects are association-based scenarios, not causal intervention estimates.

### 8. Long-Term Annual Forecast Layer

The dashboard also includes an annual planning layer for 5-, 10-, and 20-year horizons. Instead of projecting total annual utilization, this layer shifts a baseline year of daily maximum temperatures forward, recomputes endpoint-specific heat dose, scores the existing daily endpoint HVI models, and aggregates added heat-attributable burden to annual community-area outputs. Forecast exports are generated by [code/13_build_long_term_forecast_exports.R](code/13_build_long_term_forecast_exports.R).

The resulting frontend exports are intended for long-term planning views: added heat-attributable endpoint burden, heat-burden rate changes, overall forecast risk, and dominant endpoint under trend-based and fixed-warming temperature scenarios. Public burden fields are constrained to nonnegative added heat burden; signed model changes and model-boundary flags are retained for QA. These forecasts are planning scenarios, not deterministic climate projections or daily alert forecasts.
