...
Scenario exports are generated by [code/12_build_scenario_exports.R](code/12_build_scenario_exports.R). The script converts Fahrenheit to Celsius for model scoring, holds humidity at baseline by default, applies NDVI changes and AC loss-of-cooling scenarios from each community area's baseline, caps heat dose at endpoint-specific observed support, and marks model-boundary rows using `heat_dose_capped` and `prediction_capped_for_display`.
